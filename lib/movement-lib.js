...
exports.randomSID = function(airport, runway) {
    const rh = runway.substring(0, 2)
    const sids = airport.sid[rh]
    return sids[Math.floor(Math.random() * sids.length)]
}

exports.randomSTAR = function(airport, runway) {
    const rh = runway.substring(0, 2)
    const stars = airport.star[rh]
...
exports.takeoff = function(airport, aircraft_model, parking_name, runway_name, sid_name) {
    ...
    if (p2) {
        ...
        airplane.addMarker(
            p1,
            common.to_kmh(aircraft.taxi_speed),
            hold_time,
            syncCount++,
            airplane.getProp("color"),
            {
    ...
    if (p) {
        ...
        airplane.addMarker(
            p1,
            common.to_kmh(aircraft.climbspeed1),
            null,
            syncCount++,
            airplane.getProp("color"),
            {
...
exports.land = function(airport, aircraft_model, parking_name, runway_name, star_name) {
    ...
    if (p) {
        ...
        p.geometry.coordinates.forEach(function(c, idx) {
            if (first) {
                ...
                airplane.addMarker(
                    c,
                    common.to_kmh(aircraft.vinitialdescend),
                    null,
                    syncCount++,
                    airplane.getProp("color"),
                    {
    ...
    if (p) {
        ...
        airplane.addMarker(
            p,
            common.to_kmh(aircraft.vapproach),
            null,
            syncCount++,
            airplane.getProp("color"),
            {
    ...
    if (p) {
        ...
        airplane.addMarker(
            p,
            common.to_kmh(aircraft.vlanding),
            null,
            syncCount++,
            airplane.getProp("color"),
            {
    ...
    if (p) {
        ...
        airplane.addMarker(
            p,
            common.to_kmh(aircraft.taxispeed),
            null,
            syncCount++,
            airplane.getProp("color"),
            {
