...
exports.randomRunway = function(wind) {
    var runway = _airport.runways.length > 1 ? _airport.runways[Math.floor(Math.random() * _airport.runways.length)] : _airport.runways[0]
...
exports.randomParking = function(apron = false) {
    ...
    if(apron) {
        ...
        t.forEach(function(c, idx) {
            if(_airport.parkings.features[c].apron == apron)
                ret.push(c)
        })
    } else {
    ...
    var p = ret[Math.floor(Math.random() * ret.length)]
