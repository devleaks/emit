...
function findSyncVertex(p, vertices) {
...
function emit(args) { //s=(s)tart, (e)dge, (v)ertex, (f)inish
    ...
    if ( (k == 's' || k == 'e')             // normal emit: (S)tart or on (E)dge
         ||
         (k == 'w' && !args.options.quiet)  // Paused. Does not emit if silent
         ||
         (k == 'v' /*&& args.options.vertices*/) // At vertex: If there is a marker on the vertex, we add a point it with sync info.
         ||                                      // For this point, emit is true or false whether we should emit this vertex.
         (k == 'f' /*&& args.options.lastPoint*/)// (note: the last point is also a vertex, and has an extra option to emit it.)
       ) {
        ...
        var properties = geojson.mergeProperties({ // we first merge "our" properties from here
            "emit": (k == 'v' && !args.options.vertices) || (k == 'f' && !args.options.lastPoint) ? false : true,
            "marker-color": color,
            "marker-size": "medium",
            "marker-symbol": "",
            "nojitter": args.p,
        ...
        if (k == 'v' || k == 'f') {
            var syncprop = lookupSyncAtVertex(args.newls.length, args.options._syncers, args.idx)
        ...
        //var xtraprops = getClosestProps(args.options, args.idx) // we then eventually add props from previous marker
...
function lookupSyncAtVertex(newidx, syncVertices, idx) {
    ...
    if (f && f.hasOwnProperty("properties") && f.properties.hasOwnProperty("sync")) {
        syncprop = { // sync point are just there for synchronisation, we don't need to write speed, etc.
...
exports.emitLineStringFeature = function(f, options) {
    var deviceName = (f.hasOwnProperty("properties") && f.properties.hasOwnProperty("device")) ? f.properties.device : "noname"
    ...
    while (lsidx < ls.length - 1) { // note: currpos is between ls[lsidx] and ls[lsidx+1]
        ...
        if ((to_next_emit < rate) && (to_next_emit > 0) && (timeleft2vtx < to_next_emit)) { // may be portion of segment left
            ...
            var timing = pauseAtVertex(options, { "time": time, "left": to_next_emit }, pauses[lsidx + 1] ? pauses[lsidx + 1] : null, rate, newls, nextvtx, lsidx + 1, ls.length, points, speeds, deviceName, ls)
            time = timing.time
