...
function emit(options, newls, t, p, s, pts, spd, cmt, idx, alt = false) { //s=(s)tart, (e)dge, (v)ertex, (f)inish
    var k = s.charAt(0)
    if (   (k == 's' || k == 'e')                   // normal emit
        || (k == 'w' && !options.silent)            // stopped. does not emit if silent
        || (k == 'v' && options.vertices)           // at vertice, only emit if requested
        || (k == 'f' && options.lastPoint)  ) {     // at last point, only emit if requested
        ...
        const jp = jitter(p, options.jitter)

        if(options.altitude && alt) {
    ...
    if (pause && pause > 0) {
        ...
        if (pause < rate) {
            if (pause > timing.left) { // will emit here
                emit(options, newls, timing.time + timing.left, pos, 'w', points, 0, (lsidx == (lsmax - 1)) ? "at last vertex while pauseing " + counter : "at vertex while pauseing " + counter, lsidx) // vertex
                counter++
            ...
            while (totpause > 0) {
                timing.time += rate
                emit(options, newls, timing.time, pos, 'w', points, 0, (lsidx == (lsmax - 1)) ? "at last vertex while pauseing " + counter : "at vertex while pauseing " + counter, lsidx) // vertex
                counter++
...
exports.emitLineStringFeature = function(f, options) {
    ...
    emit(options, newls, time, currpos, 's', points, speeds[0], "start", lsidx) // emit it
    ...
    while (lsidx < ls.length - 1) { // note: currpos is between ls[lsidx] and ls[lsidx+1]
        ...
        if ((to_next_emit > 0) && (to_next_emit < rate) && (timeleft2vtx > to_next_emit)) { // If next vertex far away, we move during to_next_emit on edge and emit
            ...
            emit(options, newls, time, p, 'e', points, get_speed(p, lsidx, ls, speeds), "moving from vertex with time remaining", lsidx)
        ...
        if ((to_next_emit < rate) && (to_next_emit > 0) && (timeleft2vtx < to_next_emit)) { // may be portion of segment left
            ...
            time += timeleft2vtx
            emit(options, newls, time, nextvtx, (lsidx == (ls.length - 2)) ? 'f' : 'v' + (lsidx + 1), points, speeds[lsidx + 1], "moving on edge with time remaining to next vertex", lsidx)
            currpos = nextvtx
            ...
            while (rate < timeleft2vtx) { // we will report position(s) along the edge before reaching the vertex
                ...
                emit(options, newls, time, p, 'e', points, get_speed(p, lsidx, ls, speeds), "en route", lsidx)
            ...
            if (timeleft2vtx > 0) { // may be portion of segment left
                ...
                time += timeleft2vtx
                emit(options, newls, time, nextvtx, (lsidx == (ls.length - 2)) ? 'f' : 'v' + (lsidx + 1), points, speeds[lsidx + 1], (lsidx == (ls.length - 2)) ? "at last vertex" : "at vertex", lsidx) // vertex
                currpos = nextvtx
