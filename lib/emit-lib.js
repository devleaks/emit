...
function emit(args) { //s=(s)tart, (e)dge, (v)ertex, (f)inish
    ...
    if ((k == 's' || k == 'e') // normal emit
        ||
        (k == 'w' && !args.options.quiet) // stopped. does not emit if silent
        ||
 ...
 */
function pauseAtVertex(options, timing, pause, rate, newls, pos, lsidx, lsmax, points, speeds, name) {
...
exports.emitLineStringFeature = function(f, options) {
    var deviceName = (f.hasOwnProperty("properties") && f.properties.hasOwnProperty("name")) ? f.properties.name : "noname"
    ...
    while (lsidx < ls.length - 1) { // note: currpos is between ls[lsidx] and ls[lsidx+1]
        ...
        if ((to_next_emit < rate) && (to_next_emit > 0) && (timeleft2vtx < to_next_emit)) { // may be portion of segment left
            ...
            var timing = pauseAtVertex(options, { "time": time, "left": to_next_emit }, pauses[lsidx + 1] ? pauses[lsidx + 1] : null, rate, newls, nextvtx, lsidx + 1, ls.length, points, speeds, deviceName)
            time = timing.time
