...
function lookupSyncAtVertex(newidx, syncVertices, idx) {
    ...
    if (f && f.hasOwnProperty("properties") && f.properties.hasOwnProperty("sync") && !f.properties.hasOwnProperty("synced")) {
        var allProps = geojson.cleanCopy(f.properties)
...
function emit(args) { //s=(s)tart, (e)dge, (v)ertex, (f)inish
    var k = args.s.charAt(0)    // markers are at vertices which cannnot occur on edges.
    var syncprop = k != 'e' ? lookupSyncAtVertex(args.newls.length-1, args.options._syncers, args.idx) : {}
...
exports.emitGeoJSON = function(f, options) {
    ...
    if (f.type == "FeatureCollection") {
        ...
        var fret = exports.emitLineStringFeature({
            "type": "Feature",
            "geometry": f
        }, options)
