...
function lookupSyncAtVertex(newidx, syncVertices, idx) {
    ...
    if (f && f.hasOwnProperty("properties") && f.properties.hasOwnProperty("sync")) {
        var allProps = JSON.parse(JSON.stringify(f.properties))
        const remove = ["marker","nearestPointOnLine","idx"]
...
function emit(args) { //s=(s)tart, (e)dge, (v)ertex, (f)inish
    ...
    var syncprop = (k == 'v' || k == 'f') ? lookupSyncAtVertex(args.newls.length-1, args.options._syncers, args.idx) : {}
    var hasSync = syncprop.hasOwnProperty("sync")
    if ((k == 's' || k == 'e') // normal emit: (S)tart or on (E)dge
        ...
        (k == 'f')                          // For this point, emit is true or false whether we should emit this vertex. 
        ...
        console.log(args.idx, args.s)
        var properties = geojson.mergeProperties({ // we first merge "our" properties from here
            "emit": !((k == 'v' && !args.options.vertices) || (k == 'f' && !args.options.lastPoint)),
            "marker-color": color,
            "marker-size": "medium",
            "marker-symbol": "",
            "nojitter": args.p,
