...
exports.leave = function(roads, truck_name, truck_model, parking_name, to_border) {
    ...
    truck.addMarker(
        ...
        truck.getProp("color"), {
            "device": truck.name,
            "comment": "leaves parking " + parking.properties.name
        })
    ...
    truck.addMarker(
        ...
        truck.getProp("color"), {
            "device": truck.name,
            "comment": 'enters highway ' + highway_entry
        })
    ...
    truck.addMarker(
        ...
        truck.getProp("color"), {
            "device": truck.name,
            "comment": 'exits Belgium' + to_border
        })
...
exports.arrive = function(roads, truck_name, truck_model, parking_name, from_border) {
    ...
    var first = true
    if (p) { // @todo: Add line string?
        p.geometry.coordinates.reverse().forEach(function(c, idx) {
            if (first) {
                first = false
                ...
                truck.addMarker(
                    ...
                    truck.getProp("color"), {
                        "device": truck.name,
                        "comment": 'enter belgium ' + from_border
                    })
            } else {
    ...
    if (p) {
        ...
        truck.addMarker(
            ...
            truck.getProp("color"), {
                "device": truck.name,
                "comment": 'exits highway ' + highway_exit
            })
    } else {
    ...
    truck.addMarker(
        ...
        truck.getProp("color"), {
            "device": truck.name,
            "comment": "parks " + parking.properties.name
        })
